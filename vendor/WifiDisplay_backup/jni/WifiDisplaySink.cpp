...
status_t WifiDisplaySink::sendSetup(int32_t sessionID, const char *uri) {
    mRTPSink = new RTPSink(mNetSession, mSurface);
...
#endif
    if (mRTPSink != NULL) {
        if (mRTPSink->get_mRTCPSessionID() != 0 && mRTPSink->get_mNetSession() != NULL) {
            mRTPSink->get_mNetSession()->destroySession(mRTPSink->get_mRTCPSessionID());
        }
        if (mRTPSink->get_mRTPSessionID() != 0 && mRTPSink->get_mNetSession() != NULL) {
            mRTPSink->get_mNetSession()->destroySession(mRTPSink->get_mRTPSessionID());
        }
        if (looper() != NULL) {
            looper()->unregisterHandler(mRTPSink->id());
