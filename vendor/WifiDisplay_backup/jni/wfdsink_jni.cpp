...
static Mutex gSinkLock;
static sp<WifiDisplaySink> gSink;
static sp<ALooper> gLooper;
static sp<ANetworkSession> gSession;
...
int startWFDSink(const char *devaddr, bool isgo, const sp<Surface> &surface = NULL) {
         int32_t iCount =0;
    ...
    {
        Mutex::Autolock autoLock(gSinkLock);
        gSink = sink;
        gLooper = looper;
        gSession = session;
    }
...
static jint android_p2p_startWFDSink(JNIEnv *env, jobject *thiz, jstring peerinfo, jboolean go) {
   ...
   int ret = startWFDSink(peerinfoStr, go);
   env->ReleaseStringUTFChars(peerinfo, peerinfoStr);
   return ret;
}

static jint android_p2p_startWFDSinkWithSurface(JNIEnv *env, jobject *thiz,
        jstring peerinfo, jboolean go, jobject surfaceObj) {
   UNUSED(thiz);
   const char *peerinfoStr = env->GetStringUTFChars(peerinfo, NULL);
   if (peerinfoStr == NULL) {
        return -1;
   ...
   if (surface == NULL) {
        env->ReleaseStringUTFChars(peerinfo, peerinfoStr);
        ALOGE("native_startWFDSinkWithSurface: invalid Surface");
        return -1;
   }
   int ret = startWFDSink(peerinfoStr, go, surface);
   env->ReleaseStringUTFChars(peerinfo, peerinfoStr);
   return ret;
}

static void android_p2p_stopWFDSink(JNIEnv *env, jobject *thiz) {
    UNUSED(env);
    UNUSED(thiz);
...
static const JNINativeMethod gMethods[] = {
    ...
    { "native_startWFDSinkWithSurface", "(Ljava/lang/String;ZLandroid/view/Surface;)I",
      (void *)android_p2p_startWFDSinkWithSurface },
    { "native_stopWFDSink", "()V",
      (void *)android_p2p_stopWFDSink },
};
