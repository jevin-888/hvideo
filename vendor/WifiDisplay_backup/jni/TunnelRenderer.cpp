...
void TunnelRenderer::StreamSource::disconnect() {
    Mutex::Autolock autoLock(mLock);
    mOwner = NULL;
    mListener.clear();
    mIndicesAvailable.clear();
}
    ...
    while (!mIndicesAvailable.empty()) {
		...
		do
		{
			...
			if (mNumDeqeued == 1) {
				...
				extra->setInt64("timeUs", ALooper::GetNowUs());
				#else
				// DISCONTINUITY_ABSOLUTE_TIME expects a real-time anchor, not the
				// MPEG-TS media PTS. Feeding PTS here makes MediaPlayer chase the
				// stream and can render wireless mirroring too fast.
	            extra->setInt64("timeUs", ALooper::GetNowUs());
				...
				last_tunnelrender_start_time_us = tunnelrender_start_time_us;
                                ...
                                extra->setInt64("timeUs", ALooper::GetNowUs());
                                #else
                                // DISCONTINUITY_ABSOLUTE_TIME expects a real-time anchor,
                                // not the MPEG-TS media PTS.
                                extra->setInt64("timeUs", ALooper::GetNowUs());
...
void TunnelRenderer::initPlayer() {
    if (mExternalSurface != NULL) {
        mSurface = mExternalSurface;
        ALOGI("initPlayer: using external app Surface, skip top-level SurfaceControl");
    } else {
