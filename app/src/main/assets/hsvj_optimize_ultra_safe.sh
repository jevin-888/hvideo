...
optimize_for_video() {
    ...
    # HSVJ swapchain pacing: keep the proven FIFO path and expose frame-rate
    # hints to SurfaceFlinger; async acquire/present remains enabled.
...
disable_safe_services() {
    ...
    echo "[HSVJ-SAFE] Camera services disabled"
    ...
    echo "[HSVJ-SAFE] Safe services disabled (camera + telephony confirmed by user)"
}
