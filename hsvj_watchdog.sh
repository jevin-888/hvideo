...
MAIN_ACTIVITY="com.hsvj.engine.MainActivity"
...
start_activity_direct() {
    cmd activity start-activity -n "$PACKAGE_NAME/$MAIN_ACTIVITY" --user 0 >> "$LOG_FILE" 2>&1
    sleep 2
    if ! is_app_running; then
        echo "$(date '+%F %T') activity start failed, try monkey" >> "$LOG_FILE"
        monkey -p "$PACKAGE_NAME" -c android.intent.category.LAUNCHER 1 >> "$LOG_FILE" 2>&1
        sleep 3
    fi
}
...
restart_app() {
    ...
    if [ "$restart_loop_count" -ge "$RESTART_LOOP_MAX_COUNT" ]; then
        echo "$(date '+%F %T') restart loop detected count=$restart_loop_count window=${RESTART_LOOP_WINDOW_SECONDS}s reason=$REASON, reboot device" >> "$LOG_FILE"
        sync 2>/dev/null
        reboot
        sleep 10
...
while true; do
    ...
    if ! is_app_running; then
        echo "$(date '+%F %T') app not running, delegate restart to unified script, fail_count=$app_restart_fail_count" >> "$LOG_FILE"
        device_lost_count=0
        heartbeat_fail_count=0
        http_fail_count=0
        if [ -f "$RESTART_SCRIPT" ]; then
            ...
            echo "$(date '+%F %T') restart script missing, wait ${restart_delay}s then start activity" >> "$LOG_FILE"
        ...
        if is_app_running; then
            echo "$(date '+%F %T') app restart success" >> "$LOG_FILE"
            app_restart_fail_count=0
        else
            app_restart_fail_count=$((app_restart_fail_count + 1))
            echo "$(date '+%F %T') app restart still failed, fail_count=$app_restart_fail_count" >> "$LOG_FILE"
        fi
        continue
    fi
